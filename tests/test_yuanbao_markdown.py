...
class TestHasUnclosedFence(unittest.TestCase):
    def test_unclosed_fence(self):
        self.assertTrue(has_unclosed_fence("```python\ncode"))

    def test_closed_fence(self):
        self.assertFalse(has_unclosed_fence("```python\ncode\n```"))

    def test_empty(self):
        self.assertFalse(has_unclosed_fence(""))

    def test_no_fence(self):
        self.assertFalse(has_unclosed_fence("just some text\nno fences here"))
    ...
    def test_second_fence_unclosed(self):
        text = "```python\ncode1\n```\n\n```js\ncode2"
        self.assertTrue(has_unclosed_fence(text))
...
class TestEndsWithTableRow(unittest.TestCase):
    def test_simple_table_row(self):
        self.assertTrue(ends_with_table_row("| col1 | col2 |"))
    ...
    def test_table_row_in_middle(self):
        text = "| col1 | col2 |\nsome other text"
        self.assertFalse(ends_with_table_row(text))

    def test_empty(self):
        self.assertFalse(ends_with_table_row(""))

    def test_non_table(self):
        self.assertFalse(ends_with_table_row("just a normal line"))

    def test_only_pipe_start(self):
        self.assertFalse(ends_with_table_row("| just pipe at start"))

    def test_table_separator_row(self):
        self.assertTrue(ends_with_table_row("| --- | --- |"))
...
class TestSplitAtParagraphBoundary(unittest.TestCase):
    ...
    def test_split_at_sentence_end(self):
        ...
        head, tail = split_at_paragraph_boundary(text, 25)
        self.assertLessEqual(len(head), 25)
        self.assertEqual(head + tail, text)

    def test_forced_split_no_boundary(self):
        text = "a" * 100
        head, tail = split_at_paragraph_boundary(text, 50)
        self.assertEqual(len(head), 50)
        self.assertEqual(head + tail, text)

    def test_split_at_newline(self):
        text = "line one\nline two\nline three"
        head, tail = split_at_paragraph_boundary(text, 15)
...
class TestChunkMarkdownText(unittest.TestCase):
    def test_empty(self):
        self.assertEqual(chunk_markdown_text(""), [])

    def test_short_text_no_split(self):
        text = "hello world"
        self.assertEqual(chunk_markdown_text(text, 3000), [text])

    def test_exactly_max_chars(self):
        text = "a" * 3000
        result = chunk_markdown_text(text, 3000)
    ...
    def test_5000_chars_returns_2(self):
        ...
        result = chunk_markdown_text("a" * 5000, 3000)
    ...
    def test_code_fence_not_split(self):
        ...
        result = chunk_markdown_text(text, 3000)
        for chunk in result:
            self.assertFalse(has_unclosed_fence(chunk),
    ...
    def test_multiple_paragraphs(self):
        ...
        paragraphs = ["This is paragraph number " + str(i) + ". " * 50
                      for i in range(10)]
        text = "\n\n".join(paragraphs)
        result = chunk_markdown_text(text, 500)
    ...
    def test_returns_non_empty_strings(self):
        """所有返回的片段都应为非空字符串"""
        text = "Hello world!\n\n" * 100
        result = chunk_markdown_text(text, 100)
        for chunk in result:
...
class TestAcceptanceCriteria(unittest.TestCase):
    def test_9000_x_returns_3_chunks(self):
        """验收：chunk_markdown_text("x" * 9000, 3000) 返回 3 个片段"""
        result = chunk_markdown_text("x" * 9000, 3000)
        self.assertEqual(len(result), 3)
        for chunk in result:
            self.assertLessEqual(len(chunk), 3000)
...
def test_short_text_no_split():
    assert chunk_markdown_text("hello", 100) == ["hello"]


def test_plain_text_split():
    chunks = chunk_markdown_text("a" * 5000, 3000)
    assert len(chunks) >= 2
    for c in chunks:
        assert len(c) <= 3000
...
def test_fence_not_broken():
    """代码块不应被切断"""
    code_block = "```python\n" + "x = 1\n" * 200 + "```"
    chunks = chunk_markdown_text(code_block, 1000)
    for c in chunks:
        assert not has_unclosed_fence(c), f"Chunk has unclosed fence: {c[:100]}"


def test_large_fence_kept_whole():
    """超大代码块即便超过 max_chars 也应整块输出"""
    code_block = "```python\n" + "x = 1\n" * 200 + "```"
    chunks = chunk_markdown_text(code_block, 500)
    # 代码块应在同一个 chunk 中（允许超出 max_chars）
    fence_chunks = [c for c in chunks if "```python" in c]
    for c in fence_chunks:
        assert not has_unclosed_fence(c)


def test_mixed_content():
    """代码块前后的普通文本可以正常切割"""
    text = "intro paragraph\n\n" + "```python\nx=1\n```" + "\n\noutro paragraph"
    chunks = chunk_markdown_text(text, 100)
    for c in chunks:
        assert not has_unclosed_fence(c)
...
def test_table_not_broken():
    ...
    table = "| A | B |\n|---|---|\n| 1 | 2 |\n| 3 | 4 |"
    text = "before\n\n" + table + "\n\nafter"
    chunks = chunk_markdown_text(text, 30)
...
def test_has_unclosed_fence():
    assert has_unclosed_fence("```python\ncode") == True
    assert has_unclosed_fence("```python\ncode\n```") == False
    assert has_unclosed_fence("no fence") == False


def test_ends_with_table_row():
    assert ends_with_table_row("| a | b |") == True
    assert ends_with_table_row("normal text") == False


def test_empty_text():
    assert chunk_markdown_text("", 100) == []
